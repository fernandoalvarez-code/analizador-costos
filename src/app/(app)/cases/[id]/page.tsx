...
export default function CaseDetailsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="bg-gray-100 text-slate-900 font-sans printable-area min-h-screen">
      ...
      {/* CONTENEDOR DEL REPORTE (Visible solo para PDF/Print) */}
      <div id="report-container" className="mx-auto w-fit bg-gray-100">
        
        {/* ================= HOJA 1 ================= */}
        <div className="bg-white relative flex flex-col p-8 mb-8 shadow-lg" style={{ width: '210mm', height: '297mm', overflow: 'hidden' }}>
            ...
            {/* Fotos (Espacio Flexible) */}
            <div className="flex-1 flex flex-col justify-center mb-4 min-h-[250px] overflow-hidden">
                 {validImages.length > 0 ? (
                    <div className="flex flex-col h-full justify-center">
                        ...
                        <div className="flex items-center justify-center h-full">
                            {validImages.length === 1 && (<div className="flex flex-col items-center justify-center w-full h-full"><div className="border-4 border-white shadow-lg rounded-lg overflow-hidden bg-white max-h-[350px] w-auto flex items-center justify-center">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={validImages[0]} alt="Evidencia" className="object-contain max-h-full max-w-full" /></div>{data.imageDescriptions?.[0] && <p className="mt-2 text-[10px] font-bold text-slate-700 bg-slate-100 px-3 py-0.5 rounded-full uppercase">{data.imageDescriptions[0]}</p>}</div>)}
        ...
        {/* ================= HOJA 2 ================= */}
        <div className="bg-white relative flex flex-col p-8 shadow-lg" style={{ width: '210mm', height: '297mm', overflow: 'hidden' }}>
            ...
            <div className="mb-6">
                <div className="grid grid-cols-2 gap-8 max-w-lg mx-auto">
                    ...
                    <div className="text-center"><p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Propuesta</p><div className="text-2xl font-bold text-blue-600 mb-1">{formatCurrency(r.cppB)}</div><div className="w-full border border-blue-200 rounded overflow-hidden"><div className="bg-blue-600 text-white text-[9px] py-1 font-bold">MAQ {formatCurrency(r.costoMaquinaB)}</div><div className="bg-blue-200 text-blue-900 text-[9px] py-1 font-bold">HER {formatCurrency(r.costoHerramientaB)}</div></div></div>
                </div>
            </div>

            <div className="mb-6 bg-blue-50/50 border border-blue-100 rounded-lg p-3 shadow-sm">
