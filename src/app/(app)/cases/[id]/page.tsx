...
// --- SUB-COMPONENTES (Estilo Réplica Exacta Web) ---

interface RowProps { label: string; valA: any; valB: any; bold?: boolean; isRed?: boolean; }

// Fila técnica: Fondo blanco, borde sutil, texto centrado
const Row = ({ label, valA, valB, bold = false, isRed = false }: RowProps) => (
    <div className={cn("grid grid-cols-10 border-b border-slate-200 px-3 bg-white items-center text-[10px] min-h-[26px]", bold && "font-bold bg-slate-50")}>
        <div className="col-span-4 font-medium text-slate-600 text-left leading-tight py-1">{label}</div>
        <div className={cn("col-span-3 text-center leading-tight py-1", isRed ? "text-[#D93025] font-bold" : "text-slate-700")}>{valA}</div>
        <div className={cn("col-span-3 text-center leading-tight py-1", isRed ? "text-[#1A73E8] font-bold" : "text-slate-700")}>{valB}</div>
    </div>
);

// Título de Sección: Fondo gris claro, texto oscuro
const SectionTitle = ({ title }: { title: string }) => (
    <div className="bg-slate-100 px-3 py-1.5 text-[9px] font-bold text-slate-500 uppercase border-y border-slate-300 text-left tracking-wider">{title}</div>
);

interface FinancialRowProps { label: string; valA: any; valB: any; save: any; pct: any; }

// Fila Financiera: Estilo limpio
const FinancialRow = ({ label, valA, valB, save, pct }: FinancialRowProps) => (
    <div className="grid grid-cols-12 border-b border-green-100 px-3 bg-white items-center text-center text-[10px] min-h-[28px]">
        <div className="col-span-3 font-medium text-slate-600 text-left leading-tight py-1">{label}</div>
        <div className="col-span-2 text-slate-600 leading-tight py-1">{valA}</div>
        <div className="col-span-2 text-slate-600 leading-tight py-1">{valB}</div>
        <div className="col-span-3 font-bold text-[#137333] leading-tight py-1">{save}</div>
        <div className="col-span-2 text-[#137333] font-bold leading-tight py-1">{pct}</div>
    </div>
);
...
export default function CaseDetailsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="bg-gray-100 text-slate-900 font-sans printable-area min-h-screen">
      <style jsx global>{`
        .pdf-page { width: 210mm; padding: 25px 40px; background: white; position: relative; display: flex; flex-direction: column; box-sizing: border-box; }
      ...
      <div id="report-container" className="mx-auto w-fit bg-gray-100">
        ...
        {/* ================= PÁGINA 2 (CLON EXACTO DE LA CAPTURA) ================= */}
        <div className="pdf-page">
            ...
            <div className="mb-4">
                <div className="grid grid-cols-2 gap-8 max-w-3xl mx-auto">
                    ...
                    <div className="text-center">
                        ...
                        <div className="text-4xl font-black text-[#D93025] mb-3 leading-none">{formatCurrency(r.cppA)}</div>
                        <div className="flex w-full h-8 rounded overflow-hidden shadow-sm">
                            <div className="bg-[#D93025] flex items-center justify-center text-white text-[9px] font-bold h-full" style={{ width: '70%' }}>
                                MAQ {formatCurrency(r.costoMaquinaA)}
                            </div>
                            <div className="bg-[#FAD2CF] flex items-center justify-center text-[#8C1B15] text-[9px] font-bold h-full" style={{ width: '30%' }}>
                                HER {formatCurrency(r.costoHerramientaA)}
                            </div>
                        </div>
                    </div>
                    {/* Columna Propuesta */}
                    <div className="text-center">
                        ...
                        <div className="text-4xl font-black text-[#1A73E8] mb-3 leading-none">{formatCurrency(r.cppB)}</div>
                        <div className="flex w-full h-8 rounded overflow-hidden shadow-sm">
                            <div className="bg-[#1A73E8] flex items-center justify-center text-white text-[9px] font-bold h-full" style={{ width: '70%' }}>
                                MAQ {formatCurrency(r.costoMaquinaB)}
                            </div>
                            <div className="bg-[#D2E3FC] flex items-center justify-center text-[#174EA6] text-[9px] font-bold h-full" style={{ width: '30%' }}>
                                HER {formatCurrency(r.costoHerramientaB)}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="mb-4">
                <div className="grid grid-cols-2 gap-6">
                    <div className="border border-slate-200 rounded-lg p-4 shadow-sm text-center bg-white">
                        ...
                        <p className={cn("text-4xl font-black mb-1 leading-none", r.toolCostIncreasePercent < 0 ? "text-green-600" : "text-slate-700")}>
                    ...
                    <div className="border border-slate-200 rounded-lg p-4 shadow-sm text-center bg-white">
                        <p className="text-[10px] font-bold text-slate-500 uppercase mb-2 tracking-wide">Mejora Costo Total</p>
                        <p className={cn("text-4xl font-black mb-1 leading-none", r.totalCostReductionPercent > 0 ? "text-[#1A73E8]" : "text-slate-700")}>
            ...
            {(r.inversionInicial > 0) && (
                <div className="mb-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4 shadow-sm flex items-center justify-between break-inside-avoid">
                    <div>
            ...
            <div className="mb-4 border border-slate-300 rounded-t-lg rounded-b-lg overflow-hidden text-[10px] shadow-sm break-inside-avoid">
                <div className="grid grid-cols-10 bg-[#F8F9FA] font-bold border-b border-slate-300 py-2 px-3 text-[10px] tracking-wide items-center">
                    <div className="col-span-4 text-slate-700">PARÁMETRO</div>
                    <div className="col-span-3 text-center text-[#D93025]">ACTUAL (A)</div>
                    <div className="col-span-3 text-center text-[#1A73E8]">PROPUESTA (B)</div>
                </div>
                ...
                <div className="grid grid-cols-10 bg-[#E8EAED] px-3 font-black border-t border-slate-300 text-[10px] tracking-wide min-h-[30px] items-center">
                    <div className="col-span-4 uppercase text-slate-800 flex items-center h-full">COSTO TOTAL / PIEZA</div>
                    <div className="col-span-3 flex items-center justify-center h-full text-[#D93025]">{formatCurrency(r.cppA)}</div>
                    <div className="col-span-3 flex items-center justify-center h-full text-[#1A73E8]">{formatCurrency(r.cppB)}</div>
                </div>
            </div>

            {/* Tabla Financiera */}
            <div className="border border-green-200 rounded-t-lg rounded-b-lg overflow-hidden text-[10px] shadow-sm break-inside-avoid">
                <div className="grid grid-cols-12 bg-[#E6F4EA] py-2 px-3 font-bold text-[#137333] border-b border-green-200 text-[10px] text-center tracking-wide items-center">
                    <div className="col-span-3 text-left">MÉTRICA</div>
                    <div className="col-span-2 text-slate-700">ACTUAL</div>
                    <div className="col-span-2 text-slate-700">PROPUESTA</div>
                ...
                <div className="grid grid-cols-12 bg-[#CEEAD6] px-3 font-black border-t border-green-300 text-center text-[10px] tracking-wide min-h-[32px] items-center">
                    ...
                    <div className="col-span-3 text-[#137333] text-sm flex items-center justify-center h-full">{formatCurrency(r.ahorroAnual)}</div>
                    <div className="col-span-2 text-[#137333] flex items-center justify-center h-full">{formatPercent(r.totalCostReductionPercent)}</div>
                </div>
            </div>
        ...
        {hasThirdPage && (
            <div className="pdf-page">
                ...
                <div className="prose prose-sm max-w-none text-justify flex-1">
                    <h2 className="text-xl font-bold text-slate-800 mb-4">Análisis y Conclusiones Adicionales</h2>
