...
// --- SUB-COMPONENTES (Calibrados para Página 2 perfecta) ---

interface RowProps { label: string; valA: any; valB: any; bold?: boolean; isRed?: boolean; }

// AJUSTE FINAL: text-[9px] con py-0.5 y min-h-[20px]. 
const Row = ({ label, valA, valB, bold = false, isRed = false }: RowProps) => (
    <div className={cn("grid grid-cols-10 border-b border-slate-200 px-2 bg-white items-center text-[9px] min-h-[20px]", bold && "font-bold bg-slate-50")}>
        <div className="col-span-4 font-medium text-slate-700 text-left leading-tight py-0.5">{label}</div>
        <div className={cn("col-span-3 text-center leading-tight py-0.5", isRed ? "text-red-600 font-bold" : "text-slate-600")}>{valA}</div>
        <div className={cn("col-span-3 text-center leading-tight py-0.5", isRed ? "text-blue-600 font-bold" : "text-slate-600")}>{valB}</div>
    </div>
);

const SectionTitle = ({ title }: { title: string }) => (
    <div className="bg-slate-100 px-2 py-0.5 text-[8px] font-bold text-slate-500 uppercase border-y border-slate-300 text-left tracking-widest">{title}</div>
);
...
const FinancialRow = ({ label, valA, valB, save, pct }: FinancialRowProps) => (
    <div className="grid grid-cols-12 border-b border-green-100 px-2 bg-white items-center text-center text-[9px] min-h-[22px]">
        <div className="col-span-3 font-medium text-slate-700 text-left leading-tight py-0.5">{label}</div>
        <div className="col-span-2 text-slate-600 leading-tight py-0.5">{valA}</div>
        <div className="col-span-2 text-slate-600 leading-tight py-0.5">{valB}</div>
        <div className="col-span-3 font-bold text-green-600 leading-tight py-0.5">{save}</div>
        <div className="col-span-2 text-green-600 font-bold leading-tight py-0.5">{pct}</div>
    </div>
);
...
export default function CaseDetailsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="bg-gray-100 text-slate-900 font-sans printable-area min-h-screen">
      <style jsx global>{`
        .pdf-page { width: 210mm; padding: 20px 35px; background: white; position: relative; display: flex; flex-direction: column; box-sizing: border-box; }
      ...
      <div id="report-container" className="mx-auto w-fit bg-gray-100">
            ...
            <div className="bg-slate-50 rounded-xl p-3 mb-3 border border-slate-200 shadow-sm">
                <div className="grid grid-cols-2 gap-y-3 gap-x-6">
                    <div className="border-b border-slate-200 pb-2">
                        <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Cliente</span>
                        <span className="block text-lg font-bold text-slate-700 truncate leading-normal pb-0.5">{data.cliente || '-'}</span>
                    </div>
                    <div className="border-b border-slate-200 pb-2">
                        <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Operación</span>
                        <span className="block text-lg font-bold text-slate-700 truncate leading-normal pb-0.5">{data.operacion || '-'}</span>
                    </div>
                    <div className="border-b border-slate-200 pb-2">
                        <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Material</span>
                        <span className="block text-lg font-bold text-slate-700 truncate leading-normal pb-0.5">{data.material || '-'}</span>
                    </div>
                    <div className="border-b border-slate-200 pb-2">
                        <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Ahorro Anual</span>
        ...
        {/* ================= PÁGINA 2 (COMPACTADA PERO LEGIBLE) ================= */}
        <div className="pdf-page">
            ...
            <div className="flex justify-between items-center mb-2 border-b border-slate-200 pb-2 h-10">
                ...
                <div className="text-right"><span className="text-[10px] text-slate-400">Página {hasThirdPage ? '2/3' : '2/2'}</span></div>
            </div>
            ...
            <div className="mb-2 border border-slate-300 rounded overflow-hidden text-[9px] shadow-sm break-inside-avoid">
                <div className="grid grid-cols-10 bg-slate-100 font-bold border-b border-slate-300 py-0.5 px-2 text-[9px]"><div className="col-span-4">PARÁMETRO</div><div className="col-span-3 text-center text-red-600">ACTUAL (A)</div><div className="col-span-3 text-center text-blue-600">PROPUESTA (B)</div></div>
                ...
                <div className="grid grid-cols-10 border-b border-slate-200 py-0.5 px-2 bg-white text-[9px]"><div className="col-span-4 font-medium text-slate-700">Insertos/Mes</div><div className="col-span-3 text-center text-slate-600">{insertosMesA.toFixed(1)} <span className="text-[9px] text-slate-400">({formatCurrency(costoInsertosMesA)})</span></div><div className="col-span-3 text-center text-slate-600">{insertosMesB.toFixed(1)} <span className="text-[9px] text-slate-400">({formatCurrency(costoInsertosMesB)})</span></div></div>
