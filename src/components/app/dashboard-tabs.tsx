...
export default function DashboardTabs({ initialData, isReadOnly = false }: DashboardTabsProps) {
  ...
  // 1. Efecto: Cálculo Automático Diagnóstico Rápido
  useEffect(() => {
    ...
    if (deltaP <= 0) { 
        setQuickResult({ breakEvenSeconds: 0, breakEvenPieces: 0, deltaP: deltaP, tcA: tcA, vcBTarget: vcA || 0, newCycleTimeTarget: tcA });
        return;
    }
    ...
    let vcBTarget = 0; 
    if (vcA && vcA > 0 && tcB_target > 0) { vcBTarget = vcA * (tcA / tcB_target); }
  ...
  // 2. Efecto: Cálculo Automático Ahorro Neto
  useEffect(() => {
    ...
    const hasImprovements = (piezasMasReales && piezasMasReales !== 0) || 
                            (segundosMenosReales && segundosMenosReales !== 0) || 
                            (modoSimulacionTiempo === 'vc' && vcBReal && vcBReal !== 0);
  ...
  // 3. Efecto: Cálculo Automático Informe Detallado
  useEffect(() => {
    const data = detailValues;
    if (!data.machineHourlyRate || !data.piezasAlMes) {
        return; 
    }
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (initialData.imageUrls) {
        setKeptImageUrls(initialData.imageUrls);
      }
      if (initialData.imageDescriptions) {
        setImageDescriptions(initialData.imageDescriptions);
      }
    }
  ...
  const handleSaveCase = async (caseName: string) => {
    ...
    try {
      ...
      if (imagesToUpload.length > 0) {
        ...
        const promises = imagesToUpload.map(async (file) => {
        ...
        <TabsContent value="detailed">
          <Card>
            <CardHeader className="no-print flex-row justify-between items-center">
              ...
              {!isReadOnly && <Button onClick={() => router.push('/dashboard')} variant="outline" size="sm">Nuevo Informe</Button>}
            </CardHeader>
            <CardContent className="space-y-6">
              <Form {...detailedForm}>
